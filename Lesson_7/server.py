...
class Server():
    ...
    def action(self, msg):
        if 'action' in msg and \
            ...
            msg = self.jim.set_clients(self.clients)
            return msg
        elif 'action' in msg and msg['action'] == 'getContacts':
            name = msg.get('client_name')
            contacts = self.repo.get_contacts(name)
            contact_list = []
            ...
            msg = self.jim.set_contacts(name, contact_list)
            return msg
        elif 'action' in msg and msg['action'] == 'msg_to_user':
            from_name = msg.get('from')
            to_name = msg.get('to')
            msg = msg.get('message')
            sock = self.user_and_sock.get(to_name)
            msg_jim = self.jim.create_msg_to_user(from_name, msg)
            self.jim.send_msg(sock, msg_jim)
            return None
        else:
            return None
